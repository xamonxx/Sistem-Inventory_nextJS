...
import { ngAnalisa, ngLaporanKonsumen, type NgReportRange } from "@/lib/ngReports";
import { NgLaporanClient } from "./NgLaporanClient";
...
export default async function NonGudangLaporanPage({
  searchParams,
}: {
    ...
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-900/50 dark:bg-red-950/20">
        ...
        <p className="mt-2 text-sm text-red-700 dark:text-red-400">
          Laporan non-gudang hanya diizinkan untuk <strong>Admin Non-Gudang</strong>.
        </p>
      </div>
    );
  }
  ...
  const [analisa, perKonsumen] = await Promise.all([ngAnalisa(range), ngLaporanKonsumen(range)]);

  return (
    <NgLaporanClient
      analisa={analisa}
      perKonsumen={perKonsumen}
      userName={user.nama}
