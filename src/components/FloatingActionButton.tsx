...
export function FloatingActionButton({ role }: { role?: string }) {
  ...
  const allActions = [
    { label: "POS Transaksi Baru", icon: ShoppingCart, color: "text-orange-600 bg-orange-50", link: "/kasir", roles: ["ADMIN_KASIR"] },
  ...
  return (
    <div ref={menuRef} className="no-print pointer-events-none fixed bottom-6 right-6 z-40 flex flex-col items-end">
        ...
        {actions.map((act) => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-3 rounded-full bg-white px-4 py-2.5 shadow-lg border border-border hover:bg-slate-50 transition cursor-pointer"
            >
      ...
      <button
        ...
        className={cn(
          "pointer-events-auto flex h-14 w-14 items-center justify-center rounded-full bg-[var(--primary)] text-white shadow-lg transition-all duration-300 hover:bg-[var(--primary-strong)] active:scale-95 cursor-pointer",
          isOpen ? "rotate-45" : ""
        )}
