...
async function main() {
  ...
  // Fetch all items for ID mapping
  const allItems = await prisma.item.findMany({ orderBy: { id: "asc" } });
  const itemsById = new Map(allItems.map((it) => [it.kode, it]));

  // Create initial stock ledger entries in bulk
  console.log("  📋 Creating initial stock ledger entries...");
  const initialLedgers: any[] = [];
  for (const item of allItems) {
    initialLedgers.push({
      itemId: item.id,
      tanggal: new Date("2026-01-01T08:00:00Z"),
      tipe: "MASUK" as const,
      qty: item.stokAwal,
      keterangan: "Stok Awal (Sistem Seeding)",
      refType: "MANUAL",
      userId: userGudang.id,
    });
  }

  for (let chunk = 0; chunk < initialLedgers.length; chunk += 500) {
    const batch = initialLedgers.slice(chunk, chunk + 500);
    await prisma.stockLedger.createMany({ data: batch });
  }
  console.log(`  ✅ ${allItems.length} items + initial stock ledgers seeded.\n`);
  ...
  console.log("- Kasir:  username 'kasir'  / password 'password'");
  console.log("- Gudang: username 'gudang' / password 'password'");
  console.log("=========================================");
}
